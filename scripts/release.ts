 ...
 * Release helper.
 *
 *   bun scripts/release.ts version [--dry]  Aggregate the conventional commits since the tag
 *                                           for the current version and write the next version
 *                                           to the root and every workspace package.json
 *                                           (nothing if no commit asks for a bump).
 */
import { $ } from "bun";

/** Conventional types that release. Anything else (chore, docs, ci, ...) ships nothing. */
const BUMPS = { feat: "minor", fix: "patch", perf: "patch" } as const;
/** Recognised types, so prose like "warning!: ..." can't be read as a breaking change. */
const TYPES = new Set([
  "build",
  "chore",
  "ci",
...
/** `feat(scope)!: subject`, also as a squash bullet: `* feat!: subject` */
...
/** The root manifest holds the version the whole workspace is pinned to. */
...
/** Plain semver increment, including 0.x -> 1.0.0 on a breaking change. */
